// Print-in-place desk cable comb - v001
...
post_h       = 1.4;   // catch post height above tower top
...
module body() {
    difference() {
        ...
        for (i = [0 : n_ch-1]) {
            ...
            // hinge relief notch (left tower top, under the web)
            translate([cx(i) - tw/2 - hinge_l, -4.3, base_t + 10])
                cube([hinge_l + 0.01, 8.6, tower_h - 10 - hinge_t]);
...
module catch_post(i) {
    ...
    translate([xf, 0, 0]) {
        translate([0, -2, top_z]) cube([post_w, 4, post_h]);
