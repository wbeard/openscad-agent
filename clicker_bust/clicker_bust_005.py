"""Stylized 'Clicker' bust on a pedestal — organic SDF build, v005.

v004 -> v005:
  - mouth is now a shallow crater (was tunnelling through the face, leaving a
    donut-shaped shell); chin widened below it
  - ropy cords added to the neck surface
  - canopy brim gets positional jitter so the head silhouette is asymmetric
...
face = ellipsoid((9.5, 10, 11)).translate((0, -9, 146))
chin = ellipsoid((7.5, 6.5, 5.5)).translate((0, -13, 136))
head_base = neck | face.k(4.5) | chin.k(2.5)
# ropy cords on the neck surface
for cx, cy in ((-6, -8), (0, -10), (6, -8), (-9, -3), (9, -3)):
    cord = capsule((cx * 0.8, cy * 0.9, 116), (cx * 0.65, cy - 2, 138), 1.6)
    head_base = head_base | cord.k(1.2)
# gaping jaw: shallow crater between chin and canopy (not a through-tunnel)
mouth = ellipsoid((5, 5.5, 4.5)).rotate(-0.45, X).translate((0, -16.5, 143.5))
head_base = head_base - mouth.k(1.1)
...
for i in range(11):
    a = i / 11 * 2 * math.pi
    x = 23 * math.sin(a) + rng.uniform(-2.5, 2.5)
    y = -20 * math.cos(a) + 3 + rng.uniform(-2.5, 2.5)   # i=0 lands at the front
    z = 168 + rng.uniform(-4, 4) - 4 * math.cos(a)
    head = head | sphere(rng.uniform(6.5, 11.5)).translate((x, y, z)).k(2.6)
...
f.save('clicker_bust/clicker_bust_005.stl', samples=2**24, verbose=False)
