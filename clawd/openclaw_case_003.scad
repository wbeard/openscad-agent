...
claw_r1   = 12;    // big lobe
claw_r2   = 8;     // small lobe (creates the pincer crease)
claw_z    = 15;    // big lobe center height
claw_off  = 24;    // distance from corner circle center, along the diagonal
...
// Printed lying flat (built in the XY plane).
module antenna() {
    pts = [for (a = [0 : 15 : 90]) [18*sin(a), 18*(1 - cos(a))]];
    union() {
        // plug (goes into the hole)
        rotate([0, -90, 0]) cylinder(d = ant_hole_d - 0.4, h = 10);
        // flange (sits in the counterbore, reads as the white grommet ring)
        rotate([0, -90, 0]) cylinder(d = ant_cbore_d - 0.5, h = 1.4);
        // curved horn
        for (i = [0 : len(pts) - 2])
            hull() {
                translate([pts[i].x, pts[i].y, 0])
                    sphere(3 - 1.5*i/(len(pts)-1));
                translate([pts[i+1].x, pts[i+1].y, 0])
                    sphere(3 - 1.5*(i+1)/(len(pts)-1));
            }
    }
}
...
if (PART == "shell") {
    ...
    // antennae lie flat; raise to rest on the bed
    for (y = [-1, 1])
        translate([0, y*15, 3])
            antenna();
    for (y = [-1, 1])
