...
module claw() {
    difference() {
        hull() { sphere(9.5); translate([13, 0, 1]) sphere(6.5); }
        // pincer notch: wedge apex inside the tip, opening outward (XZ plane)
        translate([13.5, 0, 1])
            rotate([90, 0, 0])
                linear_extrude(30, center = true)
                    polygon([[0, 0], [14, 6], [14, -6]]);
    }
}
...
module shell() {
    difference() {
        union() {
            ...
            // pincer claws, one per corner, palm embedded 3mm into the wall
            for (sx = [-1, 1], sy = [-1, 1])
                translate([sx*(corner + (rc - 3)*0.707),
                           sy*(corner + (rc - 3)*0.707),
                           claw_z])
