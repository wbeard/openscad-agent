...
// Crab pincer, built pointing along +X: bulbous palm tapering to two
// rounded-pointy fingers with a notch between the tips
module claw() {
    // palm
    hull() { sphere(10); translate([6, 0, 1]) sphere(7.5); }
    // upper (fixed) finger
    hull() { translate([6, 0, 1])  sphere(7.5); translate([13, 0, 4])  sphere(4); }
    hull() { translate([13, 0, 4]) sphere(4);   translate([18, 0, 5.5]) sphere(2.2); }
    // lower (movable) finger
    hull() { translate([6, 0, -1])  sphere(6);   translate([12, 0, -4]) sphere(3.2); }
    hull() { translate([12, 0, -4]) sphere(3.2); translate([16, 0, -6]) sphere(1.8); }
}
...
module shell() {
    difference() {
        union() {
            ...
            // pincer claws, one per corner, palm centered on the wall surface
            for (sx = [-1, 1], sy = [-1, 1])
                translate([sx*(corner + rc*0.707),
                           sy*(corner + rc*0.707),
                           claw_z])
