// Monolith Sleeve - friction-fit architectural shell for the M4 Mac mini - v002
...
pier_w    = 8;                       // pier plan size; tops at plinth_gap
...
module chassis() {
    difference() {
        union() {
            intersection() {
                union() {
                    ...
                    for (s = [-1, 1])                   // 2 low spars along Y
                        translate([s*spar_x - rail_w/2, -cav_w/2, 0])
                            cube([rail_w, cav_w, string_h]);
        ...
        translate([-out_x/2 - 1, 40, -0.5])
            cube([out_x/2 - 38 + 1, 24.5, plinth_gap + 1]);
    }
}
