// Monolith Sleeve - friction-fit architectural shell for the M4 Mac mini - v001
...
deck_gap   = 2;     // clearance above the mini, below the tray
deck_t     = 4;     // tray plate thickness
bay_h      = 20;    // gear bay zone height
loft_h     = 10;    // dead air above the bays, under the cap
...
// ---- Chassis (standoff rails + baffle) ----
ring_lo  = 49;  ring_hi = 61;        // keep-clear annulus over the base ring
rail_y   = [-52, -20, 20, 52];       // 4 rails running X
rail_w   = 8;
rail_h   = plinth_gap;               // 10
arch_x   = [-38, 0, 38];             // airflow underpasses through the rails
arch_w   = 24;  arch_h = 6;
...
// ---- Keys (sleeve) / notches (chassis) - one-way drop-in ----
// Placed on the side walls at the scallop webs (the literal corners are cut
// away by the skirt scallops/cove); asymmetric Y positions forbid 180-deg flip.
key_w = 8;  key_p = 2.5;
key_y = [-16, 32];
...
module key_blocks() {
    for (s = [-1, 1], yc = key_y)
        translate([s*(cav_w/2 + (2 - key_p)/2), yc, plinth_gap/2])
            cube([key_p + 2, key_w, plinth_gap], center = true);
}
...
module chassis() {
    difference() {
        intersection() {
            union() {
                for (yc = rail_y)                       // 4 standoff rails along X
                    translate([-out_x/2, yc - rail_w/2, 0])
                        cube([out_x, rail_w, rail_h]);
                for (s = [-1, 1])                       // 2 side spars along Y
                    translate([s*spar_x - rail_w/2, -cav_w/2, 0])
                        cube([rail_w, cav_w, rail_h]);
                translate([-out_x/2, baffle_y - baffle_t/2, 0])  // plenum baffle
                    cube([out_x, baffle_t, rail_h]);
            }
            chassis_footprint();
        }
        // airflow underpasses through rails (NOT the baffle - it must seal)
        for (yc = rail_y, xc = arch_x)
            translate([xc - arch_w/2, yc - rail_w/2 - 1, -0.5])
                cube([arch_w, rail_w + 2, arch_h + 0.5]);
        // ... and through the spars, aligned with the side scallops
        for (s = [-1, 1], yc = scal_side_y)
            translate([s*spar_x - rail_w/2 - 1, yc - arch_w/2, -0.5])
                cube([rail_w + 2, arch_w, arch_h + 0.5]);
        // keep the base ring unobstructed: relieve every top over the annulus
        translate([0, 0, rail_h - 4.5])
            linear_extrude(rail_h)
                difference() { circle(ring_hi); circle(ring_lo); }
        // finger relief for the power button (matches the sleeve cove)
        translate([-out_x/2 - 1, 40, -0.5]) cube([out_x/2 - 38 + 1, 24.5, rail_h + 1]);
        // key notches (mate the sleeve side-wall keys; key face at 61.75, +0.3 clr)
        for (s = [-1, 1], yc = key_y)
            translate([s*(cav_w/2 - key_p - 0.3 + 2.275), yc, rail_h/2])
                cube([4.55, key_w + 0.6, rail_h + 2], center = true);
    }
}
