...
module vent_pattern2d() {
    difference() {
        circle(d=vent_d);
        // odd stripe count -> every family has a stripe through y=0, so the
        // three families intersect at common points = triangle tessellation
        for (a = [0, 60, 120]) rotate(a)
            ycopies(spacing=vent_pitch, n=2*ceil(vent_d/vent_pitch/2)+3)
...
module mouth_lips() {
    difference() {
        union() {
            ...
            translate([0,0,2.6]) scale([1.6, 0.72, 0.95]) torus(r_maj=6.8, r_min=3.3);
            translate([0,0,2.6]) scale([1.5, 0.55, 0.58]) sphere(r=6.8); // fill donut hole
        }
        translate([0,0,5.9]) cuboid([24, 1.1, 2.4], rounding=0.5);       // mouth slit
    }
}
...
module mouth_bill() {
    union() {
        mouth_mount();
        mouth_flange([26, 10.5]);
        // spatula bill: wide squashed root -> narrower, dipped tip
        hull() {
            for (sx=[-1,1]) translate([sx*9.2, 0.6, 2.6]) scale([1, 0.78, 1]) sphere(r=4.2);
            for (sx=[-1,1]) translate([sx*5.2, -1.6, 8.0]) scale([1, 0.72, 1]) sphere(r=2.7);
        }
    }
}
...
module mouth_teeth() {
    union() {
        mouth_mount();
        mouth_flange([26, 12]);
        hull() for (sx=[-1,1]) translate([sx*8, 4.0, 2.2]) sphere(r=2.6);  // upper lip roll
        for (sx=[-1,1])
            translate([sx*3.5, -1.4, 1.6]) cuboid([5.8, 8, 2.6], rounding=1.2);
    }
}
...
module mouth_tongue() {
    difference() {
        union() {
            ...
            hull() for (sx=[-1,1]) translate([sx*6.5, 3.2, 2.2]) sphere(r=2.5); // lip roll
            hull() {
                translate([0, 1.0, 2.0]) scale([1.15, 0.8, 0.62]) sphere(r=5.6);
                translate([0, -3.5, 3.2]) scale([1.15, 0.7, 0.5]) sphere(r=5.2);
                translate([0, -7.0, 3.8]) scale([1.1, 0.6, 0.45]) sphere(r=4.2);
            }
        }
        translate([0, -5.5, 5.8]) xrot(-16) cuboid([1.2, 10, 3], rounding=0.5); // groove
    }
}
